...
MultiCorrectGrammarExerciseWidget::MultiCorrectGrammarExerciseWidget(QWidget *parent) : QWidget(parent) {
    QVBoxLayout *mainLayout = new QVBoxLayout(this);
    mainLayout->setSpacing(10); // Add some spacing

    instructionLabel = new QLabel(tr("Select all correct options to complete/answer the statement:"));
    ...
    boldFont.setPointSize(boldFont.pointSize() + 2); // Make question text a bit larger
    questionLabel->setFont(boldFont);
    questionLabel->setStyleSheet("color: #2c3e50;"); // Dark blue-gray
    ...
    submitButton->setStyleSheet("QPushButton { background-color: #3498db; color: white; border-radius: 5px; padding: 8px; }"
    ...
    mainLayout->addWidget(submitButton, 0, Qt::AlignHCenter); // Center button
    mainLayout->addWidget(feedbackLabel);
    mainLayout->addStretch();

    connect(submitButton, &QPushButton::clicked, this, &MultiCorrectGrammarExerciseWidget::submitClicked);
...
void MultiCorrectGrammarExerciseWidget::setTask(const MultiCorrectGrammarTask& task) {
    ...
    feedbackLabel->clear();
    feedbackLabel->setStyleSheet(""); // Reset stylesheet

    // Clear previous check boxes
    ...
    for (const QString& optionText : task.options) {
        ...
        cb->setStyleSheet("QCheckBox { spacing: 5px; } QCheckBox::indicator { width: 15px; height: 15px; }");
        checkBoxes.append(cb);
        optionsLayout->addWidget(cb);
    }
    if(!checkBoxes.isEmpty()) checkBoxes.first()->setFocus();
}
...
void MultiCorrectGrammarExerciseWidget::showCorrectAnswers(const QSet<int>& correctIndices, const QStringList& options) {
    QString correctText = tr("Correct options were: ");
    bool first = true;
    for (int index : correctIndices) {
        if (index >= 0 && index < options.size()) {
            if (!first) correctText += ", ";
            correctText += "\"" + options[index] + "\"";
            first = false;
        }
    }
    feedbackLabel->setText(correctText);
    feedbackLabel->setStyleSheet("color: #2980b9; font-weight: bold;"); // Informative blue
}
